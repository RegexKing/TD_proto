...
package org.flixel.plugin.photonstorm
{
	...
	public class FlxButtonPlus extends FlxGroup
	{
		 ...
		 */
		public function FlxButtonPlus(X:int, Y:int, Callback:Function, Params:Array = null, Label:String = null, colorVisible:Boolean = false, Width:int = 100, Height:int = 20):void
		{
			super(4);
			...
			{
				var border:FlxExtendedSprite = new FlxExtendedSprite(X, Y);
				border.makeGraphic(Width, Height, 0xff0000FF);
				border.alpha = 0.5;
				border.x = 350;
				border.y = 330;
				add(border);
			...
			if (Label != null)
			{
				...
				if (colorVisible) textNormal.setFormat(null, 8, 0xffffffff, "center", 0xff000000);
				else textNormal.setFormat(null, 12, 0xffffffff, "center", 0xff000000);
				textNormal.scrollFactor.x = textNormal.scrollFactor.y = 0;
				
				textHighlight = new FlxText(X, Y + 3, Width, Label);
				if (colorVisible) textHighlight.setFormat(null, 8, 0xffffffff, "center", 0xff000000);
				else textHighlight.setFormat(null, 12, 0xffffffff, "center", 0xff000000);
				textHighlight.scrollFactor.x = textHighlight.scrollFactor.y = 0;
